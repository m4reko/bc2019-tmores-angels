  ...
  turn: self => {
    ...
    if (!self.macro) {
      ...
      if (self.karbonite < self.fuel) {
        self.macro = macroTasks[0]; // Set to karbonite-miner
      } else {
        self.macro = macroTasks[1]; // Set to fuel-miner
      }

      if (self.me.id%5 == 0 && self.karbonite > 50 && self.fuel>200) {
        self.task = microTasks[self.macro][1]; // Set to build church
      }else{
        self.task = microTasks[self.macro][0]; // Set to mine
      }
    ...
    if (!self.destination) {
      ...
      if(self.macro === 'mine_karbonite' || self.macro === 'mine_fuel'){
        if (self.task === 'mine') {
    ...
    if (self.task === 'stash') {
      if (distanceToDestination <= 2) {
        ...
        self.log("Leaving of resources, now back to mining");
        self.task = microTasks[self.macro][0]; // Set task to mine
        self.distanceMap = unitHelper.createDistanceMap(self.destination, self.map, self.getVisibleRobotMap());
    ...
    // mine stuff
      ...
      if (location.x === self.destination.x && location.y === self.destination.y) {
        ...
        if (self.me.karbonite > 18 || self.me.fuel > 90) {
          // Walking home now
          self.task = microTasks[self.macro][2]; // set task to stash
          self.log("Done mining, going to castle now");
    ...
    // go build a church
      ...
      if (distanceToDestination <= 2) {
        // seems to have to build from straight (not diagonal) directions
        ...
        if (self.karbonite >= 50 && self.fuel >= 200) {
          ...
          if (self.me.karbonite > 18 || self.me.fuel > 90) {
            self.task = microTasks[self.macro][2]; // stash resources
            self.destination = {x: self.castle.x, y: self.castle.y};
            self.distanceMap = unitHelper.createDistanceMap(self.destination, self.map, self.getVisibleRobotMap());
          } else {
            self.task = microTasks[self.macro][0]; // mine resources
            if (self.macro === 'mine_karbonite') {
              self.destination = unitHelper.getClosestUnoccupiedKarbonite(location, self.getKarboniteMap(), self.getVisibleRobotMap());
            } else if (self.macro === 'mine_fuel') {
              self.destination = unitHelper.getClosestUnoccupiedKarbonite(location, self.getFuelMap(), self.getVisibleRobotMap());
      ...
      } else if (unitHelper.sqDist(location, unitHelper.getClosestChurch(location, self.getVisibleRobots().filter(r => r.team === self.me.team && r.type === SPECS.CHURCH))) <= 25) {

        self.task = microTasks[self.macro][0]; // mine resources instead
        if (self.macro === 'mine_karbonite') {
          self.destination = unitHelper.getClosestUnoccupiedKarbonite(location, self.getKarboniteMap(), self.getVisibleRobotMap());
        } else if (self.macro === 'mine_fuel') {
          self.destination = unitHelper.getClosestUnoccupiedKarbonite(location, self.getFuelMap(), self.getVisibleRobotMap());
    ...
    if (self.distanceMap) {
      ...
      if (self.getVisibleRobotMap()[location.y + nextDirection.y][location.x + nextDirection.x]) {
        // Reload map and direction if someone is blocking way
        self.distanceMap = unitHelper.createDistanceMap(self.destination, self.map, self.getVisibleRobotMap());
