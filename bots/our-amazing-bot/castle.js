...
var castleHelper = {
  turn: self => {
    ...
    if (self.karbonite >= 25 + self.SK && self.fuel >= 50 + self.SF && (self.spawnedProphets < ((self.step - self.step % 50) / 50 + 1)) && self.spawnedProphets < 15 && self.spawnedKarbonite > 0 && self.spawnedFuel > 0) {
      let location = {x: self.me.x, y: self.me.y};
      let possibleDirections = structureHelper.getPossibleDirections(location, self.map, self.getVisibleRobotMap())
